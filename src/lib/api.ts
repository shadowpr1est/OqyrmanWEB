  ...
  if (res.status === 401 && retry) {
    ...
    if (!isRefreshing) {
      ...
      if (!newToken) {
        ...
        refreshQueue = [];
        isRefreshing = false;
        throw new ApiException({ code: "unauthorized", message: "Session expired" }, 401);
      }
      ...
      return new Promise((resolve, reject) => {
        refreshQueue.push((newToken) => {
          if (!newToken) return reject(new ApiException({ code: "unauthorized", message: "Session expired" }, 401));
